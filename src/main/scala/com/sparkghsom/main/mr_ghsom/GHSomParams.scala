...
class GHSomParams() extends Serializable {
  ...
  def trainAndTestRegression(dataset : RDD[Instance],
                             ...
                             tau1: Double,
                             tau2: Double,
                             epochsValue: Int): (RDD[(Double, Double)],Double,Double) = {
      ...
      if(currentDataset.count()>0) {
        ...
        do {
          ...
          if (needsTraining && currentLayer.totalNeurons < instanceCount/2) {
            var fastGrowth = true
            println("Diff from criterion: " + abs(currentLayer.parentNeuron.mqe  * tau1 - mqe_m))
            if (GHSomConfig.mqe_criterion) {
              if (abs(currentLayer.parentNeuron.mqe * tau1 - mqe_m) < 0.05)
                fastGrowth = false
            }
            else {
              if (abs(currentLayer.parentNeuron.qe * tau1 - mqe_m) < 0.05)
                fastGrowth = false
            }
...
*/
  ...
  def trainAndTest(dataset : RDD[Instance],
                   ...
                   tau1: Double,
                   tau2: Double,
                   epochsValue : Int): (RDD[(Double, Double)],Double,Double,Double) = {
    ...
    while(!layerQueue.isEmpty) {
      ...
      if (GHSomConfig.class_labels) {
        ...
        println("Assigning class for neurons...")
        //classAssignments.take(5).foreach(println)
        ...
        println("Making predicitons for current layer ...")
        /*predictions.take(10).foreach(x => {
          val actualClass = groundTruth.filter(g => g._1 == x._1.id).map(e => e._2).take(1).mkString
          println("actual: " + actualClass + " distance: " +  x._2 + " predicted: " + x._3)
        })*/
    ...
    /*
      ****** STRATEGY II ******
    val bestPredsX: RDD[(Int, Iterable[String])] = MLP.map({
      case((id, distance), classAssigned) => {
        (id,classAssigned)
      }
    }).groupByKey()

    val pairRddPredictions: RDD[(Int, String)] = bestPredsX.map(e => {
      val majLabel: String = e._2.groupBy(l => l).map(t => (t._1, t._2.size)).maxBy(x => x._2)._1
      (e._1, majLabel)
    })
       ****** END STRATEGY II ******  */
