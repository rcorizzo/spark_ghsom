...
class KddCupDatasetReader(val dataset : RDD[String]) extends Serializable{
  ...
  private def getAttributeMinMaxValuesMap( inputRDD : RDD[(Int, AttributeTypeValue)]) = {
    ...
    val numericAttributesRDD = inputRDD.filter(tuple => tuple._2.attributeType == DimensionTypeEnum.DISTANCE_HIERARCHY_NUMERIC)
        .map(tuple => {
          ...
          try {
            doubleRep = tuple._2.value.toDouble
          }
          catch {
            case e => throw new NumberFormatException("Parsing error " + tuple._1 + "; " +  tuple._2.value)
          }
