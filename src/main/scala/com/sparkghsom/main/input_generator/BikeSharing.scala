...
class BikeSharing(val dataset : RDD[String]) extends Serializable{
  ...
  val attributes = Array(
    ...
    Attribute(name = "workingday",
      index = 5,
      dimensionType = DimensionTypeEnum.DISTANCE_HIERARCHY_NUMERIC,
      randomValueFunction = DistanceHierarchyDimension.getRandomDimensionValue),

    Attribute(name = "weathersit",
      index = 6,
      dimensionType = DimensionTypeEnum.DISTANCE_HIERARCHY_NUMERIC,
      randomValueFunction = DistanceHierarchyDimension.getRandomDimensionValue),
...
object BikeSharingReader {
  ...
  def main(args : Array[String]) {
    ...
    */
    ...
    var datasetReader = new BikeSharing(dataset)
